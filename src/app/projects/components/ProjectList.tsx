...
export default function ProjectList({ projects }: { projects: Project[] }) {
    return (
        <ul className="w-full grid grid-cols-1 md:grid-cols-2 gap-4 justify-between">
            {
                projects.map((project, idx) => {
                    return (
                        <li key={idx}>
                            <div className={`w-full rounded-lg flex flex-col gap-3 px-4 overflow-hidden 
                            shadow-inner 
                             hover:bg-white-hover dark:bg-dark dark:text-white dark:hover:bg-dark-hover dark:border-slate-300/75 border-1 animated`}>
                                <div>
